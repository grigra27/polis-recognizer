...
class OCRConfig:
    ...
    def from_env(cls) -> 'OCRConfig':
        ...
        try:
            ...
            max_text_size_raw = os.getenv(
                'OCR_MAX_TEXT_LENGTH', os.getenv('OCR_MAX_TEXT_SIZE', '500000')
            )
            ...
            if min_text_threshold < 0:
                raise ValueError(
                    f"OCR_MIN_TEXT_THRESHOLD must be non-negative, got: {min_text_threshold}"
                )
    ...
    def validate(self) -> None:
        ...
        if self.min_text_threshold < 0:
            raise ValueError(
                f"min_text_threshold must be non-negative, got: {self.min_text_threshold}"
            )
